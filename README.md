...
### **Breaking changes** - V1.0.3 11-08-23 🔔

> Added support for template partials. These will be useful for globally accessed components and UI elements. To properly set up template partials, run (ok to pipe these commands):
...
> You will need to add "posthtmlrc" to the dot files in the root folder. The .posthtmlrc file should be a JSON file and should have:
~~~
...
~~~
> Note that if you nest the partials or components within a directory, you will need to adjust this folder in the config file. If you are getting a compile error stating that "no such file is found" you probably have a nesting issue in this config file. Nesting partials has not been tested so you want to place your partial in your root first. Other config options are available with posthtml. 

> Your partial does not have to declare doctype and should not be calling non-inline or head scripts or calls. You only want to declare the DOM elements you actually want to inject. Ensure your global components do not have conflicting namespaces for CSS or JS. IF YOU CALL STYLES THEY SHOULD BE FROM A GLOBALLY ACCESSIBLE STYLESHEET, OTHERWISE INLINE THEM. 

> Once your partial is set up, you want to use the DOM **"<include> </include>"** tag to inject the partial. 
